...
def _onboarding_tool_names(tools: list[dict]) -> set[str]:
    ...
    for tool in tools:
        ...
        if tool.get('tool_type') == 2 and capabilities.get('adds_cli_tools'):
...
def _normalize_steps_for_onboarding_only(plan: Plan, tools: list[dict]) -> Plan:
    onboarding_tools = _onboarding_tool_names(tools)
    if not onboarding_tools:
        return plan

    trimmed_steps = [step for step in plan.steps if step.tool_name in onboarding_tools]
    if not trimmed_steps:
        return plan
    if len(trimmed_steps) == len(plan.steps):
        return plan

    plan.steps = trimmed_steps
    suffix = ' Goal boundary enforcement removed execution steps from an onboarding-only request.'
    plan.planner_note = (plan.planner_note + suffix).strip()
    return plan
...
class ModelBackedPlanner(Planner):
    ...
    def _plan_from_payload(
        ...
        return self._normalize_plan_for_goal(plan, user_prompt, goal_hints, tools)
